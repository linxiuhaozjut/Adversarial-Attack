...
from updata import (sample_gumbel, gumbel_softmax_sample, gumbel_softmax, HuiDu, QzCodeTerson, updata_imgs, Xmin_Xmax_Ymin_Ymax,
                    get_single_img, updata_single_imgs, patch, get_patch, new_updata_single_imgs)
...
from DEimprove import DEimprove, patch_draw
...
def draw_and_save_image(image, detections, names, save_path):
    """
    Draws bounding boxes on the detected objects in the image and saves it.

    Args:
        image (np.array): Original image in numpy array format.
        detections (torch.Tensor): Detections from the model in the format (x1, y1, x2, y2, conf, cls).
...
def run(
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        #     if len(det):
        #         p = Path(paths[i])
        #         save_path = det_save_dir + str(p.name)  # save image with detections
        #         det[:, :4] = scale_boxes(im.shape[2:], det[:, :4], shapes[i][0])  # rescale boxes
        #
        #         # Load original image
        #         img = cv2.imread(str(p))
        #         # Draw and save image
        #         draw_and_save_image(img, det, names, str(save_path))
        #======================================
...
def parse_opt():
    ...
    parser.add_argument("--data", type=str, default=ROOT / "data/attacktarLLVIP.yaml", help="dataset.yaml path")
    parser.add_argument("--weights", nargs="+", type=str, default=ROOT / "runs/train/tarLLVIP50/weights/best.pt", help="model path(s)")
    ...
    print_args(vars(opt))
    return opt
det_save_dir ="./runs/images_det/wuli/"
...
def main(opt):
    ...
    if opt.task in ("train", "val", "test"):  # run normally
        ...
        run_result = run(**vars(opt))
        run1, run2, run3 = run_result
        return run1[2]
...
image_path = './infer/m3fd/DE/img'

def improve(opt):
    # 生成二维码张量流程=============================
    lowest_map = main(opt)
    print("初始最低map：",lowest_map)
    num = 20
    a = torch.Tensor(num, num, 2).fill_(0.5)

    # SGD更新===========================
    epochs = 20
    col = torch.randn(3, 20, 20)
    p = gumbel_softmax(a)
    h = HuiDu(p, num)
    h.requires_grad_(True)
    optimizer = torch.optim.SGD(params=[h], lr=0.1)
    for i in range(epochs):
        patch_draw(image_path, h, col, 6, 0.25)
        temp_map = main(opt)
        # #======================
        if lowest_map >= temp_map:
            lowest_map = temp_map
            torch.save(h, './infer/m3fd/h_col/huidu_best.pt')
            torch.save(col, './infer/m3fd/h_col/col_best.pt')
        # #======================
        rloss = 0.1 * h * get_conf
        loss = rloss.sum() / 400
        ...
        loss.backward()
        optimizer.step()
        print('灰度第：' + str(i + 1) + '次更新：')
        print(h)

    h = torch.load('./infer/m3fd/h_col/huidu_best.pt')  # best
    col.requires_grad_(True)
    optimizer = torch.optim.SGD(params=[col], lr=0.1)
    for i in range(epochs):
        patch_draw(image_path, h, col, 6, 0.25)
        temp_map = main(opt)
        # #======================
...
if __name__ == "__main__":
    ...
    # h = torch.load('./infer/m3fd/h_col/huidu_best.pt')  # best
    # col = torch.load('。/infer/m3fd/h_col/col_best.pt')  # best
    # patch_draw(image_path, h, col, 6, 0.25)
    # main(opt)
    ...
    h = HuiDu(p, num)
    col = torch.randn(3, 20, 20)
    DEimprove(image_path, 20,10, 6, 0.25, h, col)
    improve(opt)
